...
async function loaddata() {
 ...
 <div class="text-div-email"><h4>${json_response.email}</h4></div>
 ...
 <span><button  class="btn1" onclick ='deletedata("${json_response._id}")'><ion-icon name="trash-outline"></ion-icon></button></span></div>`
    ...
    dp.innerHTML = `<img src="${json_response.image}"  alt="">`;
...
async function forgotPassword(event) {
    ...
    let json_data = JSON.stringify(data);
    let response = await fetch("http://localhost:3001/forgot-password", {
        method: "POST",
        headers: {
            'Content-Type': 'application/json'
        },
...
async function forgotPassword_reset(event) {
    event.preventDefault();
    console.log("form submit clicked ..")
    let querystring = window.location.search;
    let parsed_querystring = new URLSearchParams(querystring);
    const token = parsed_querystring.get('token');
    console.log("parsed_querystring token", token);
    ...
    let json_data = JSON.stringify(data);
    try {
        let response = await fetch("http://localhost:3001/reset-password", {
            method: "PATCH",
            headers: {
